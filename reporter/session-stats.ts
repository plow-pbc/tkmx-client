...
import { MAX_BUFFER_BYTES, resolveAgentsview } from "./agentsview";
...
// A static allowlist rather than a generic JSON scrubber. If agentsview adds a
...
// Versions whose full field list has been read against this allowlist. An
// unreviewed version is NOT a reason to upload nothing: the allowlist, not the
// version, is what keeps an unreviewed field local, and failing closed on the
// number would silently stop every upload the first time agentsview bumps its
// schema — the silent skip REVIEW.md asks us to break loudly instead.
const REVIEWED_SCHEMA_VERSIONS: ReadonlySet<number> = new Set([1, 2]);

export function sanitizeSessionStats(value: unknown): SessionStatsBlob | null {
  const record = asRecord(value);
  if (!record || typeof record.schema_version !== "number") return null;
  if (!REVIEWED_SCHEMA_VERSIONS.has(record.schema_version)) {
    console.error(
      `[session-stats] agentsview stats schema v${record.schema_version} has not been reviewed by`
      + ` this client — uploading only the fields the allowlist already covers`,
    );
  }
  return sanitizeWithRule(record, SESSION_STATS_RULE) as SessionStatsBlob;
}
